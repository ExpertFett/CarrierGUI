...
emit('-- The scope is north-up; the heading vector (hook-positioned dots from')
emit('-- centre outward along the ship BRC) shows which way the boat points,')
emit('-- and rotates as the heading changes.')
emit(f'c.mShip = solidW({CX-4}, {CY-4}, 8, 8, SHIP_MARK_SKIN, 4)')
emit(f'c.lblMRcv = lbl("CV", {CX+8}, {CY-2}, 30, CarrierMark, 14)')
emit('for i = 1, 7 do')
emit('    c["hdgDot" .. i] = solidW(-300, -300, 4, 4, SHIP_MARK_SKIN, 4)')
emit('end')
emit('c.lblBrcTip = lbl("", -300, -300, 60, CarrierMark, 12)')
